...
function EditorReward() {
    ...
    const [showRewardHome, setShowRewardHome] = useState(true);
    ...
    return (
        <div className="bg-gray-100">
            {showRewardHome && (
                <div>
                    <div className="flex justify-between py-5 mx-5">
                        ...
                        <div className="">
                            <button
                                className="flex flex-row bg-priorange text-white rounded-lg hover:bg-pripurple h-10 items-center text-md w-32 justify-center"
                                onClick={() => {
                                    setShowRewardHome(false);
                    ...
                    <div className="grid grid-cols-4 w-full pb-10">
                        {rewards.map((elem) => (
                            <div className="overflow-hidden shadow-lg transform rounded-lg w-68 m-auto mb-5 text-pridark">
                                <div className="w-full block h-full">
                                    <img className="h-48 w-full object-cover" alt="" src={elem.image} />
                                    <BsFillTrashFill
                                    ...
                                    <button
                                        className="absolute top-3 right-3 text-white  w-20 rounded-lg h-10 bg-prigreen transition-colors duration-700 hover:bg-gray-700"
                                        onClick={() => {
                                            setShowRewardHome(false);
                                    ...
                                    <div className="bg-white w-full p-4 flex flex-col pt-4">
                                        ...
                                        <div>
                                            <h1 className={`text-md text-purple-800 h-10 mt-4 ${ShowMore}`}>
                                                {elem.description}
                                            </h1>
            ...
            {showRewardAdd && <RewardAdd setShowRewardHome={setShowRewardHome} setShowRewardAdd={setShowRewardAdd} />}
            {showRewardEdit && <RewardEdit />}
        </div>
    );
}
